<div class="p-6 bg-white rounded-lg shadow-md">
    <div class="mb-6">
        <h2 class="text-xl font-semibold text-gray-900 mb-4">Node Comparison</h2>
        
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <!-- Node Selection -->
            <div class="lg:col-span-2">
                <label for="nodes-select" class="block text-sm font-medium text-gray-700 mb-1">Select Nodes to Compare (minimum 2)</label>
                <select 
                    id="nodes-select" 
                ...
                <p class="mt-1 text-xs text-gray-500">Hold Ctrl/Cmd to select multiple nodes</p>
            </div>
        ...
        @elseif(empty($comparisonData) || count($selectedNodeIds) < 2 || empty($comparisonData['labels'] ?? []))
            <div class="flex justify-center items-center h-80 bg-gray-50 rounded-lg">
                <div class="text-center p-6">
                    ...
                    <p class="text-gray-500 text-lg">No comparison data available</p>
                    <p class="text-gray-400 mt-2">Select at least 2 nodes and a metric to compare</p>
                </div>
            </div>
        @else
            <div class="bg-white p-4 rounded-lg border border-gray-200 h-80 mb-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Min Value</th>
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Average Value</th>
                                <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Max Value</th>
                            </tr>
                        </thead>
                                ...
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <span class="text-sm font-medium text-gray-900">{{ $summary['nodeName'] }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-800">{{ $summary['min'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-800">{{ $summary['avg'] }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-gray-800">{{ $summary['max'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
